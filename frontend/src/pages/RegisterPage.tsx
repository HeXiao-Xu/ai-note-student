...
export default function RegisterPage() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-slate-100 font-serif">创建账号</h2>
      ...
      <div>
        <label className="block text-sm font-medium text-slate-400 mb-1.5">用户名</label>
        <input
          type="text"
          value={username}
      ...
      <div>
        <label className="block text-sm font-medium text-slate-400 mb-1.5">确认密码</label>
        <input
          type="password"
        ...
        {confirmPassword && password !== confirmPassword && (
          <p className="text-rose-400 text-xs mt-1.5">两次输入的密码不一致</p>
        )}
      </div>
