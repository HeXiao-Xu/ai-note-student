    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          <p className="text-slate-400 mb-3">课程不存在</p>
          <button onClick={() => navigate('/notes')} className="text-indigo-600 hover:text-indigo-500 text-sm transition-colors font-medium">返回全部笔记</button>
        </div>
      </div>
    )
  }
    ...
    <div className="flex h-full">
      ...
      <div className="w-72 border-r border-slate-200 bg-slate-50/50 flex flex-col shrink-0">
        <div className="p-4 border-b border-slate-200 shrink-0">
          <button onClick={() => navigate('/notes')} className="flex items-center gap-1 text-xs text-slate-400 hover:text-slate-600 transition-colors mb-3">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full" style={{ backgroundColor: course.color || '#6366f1' }} />
              <h2 className="text-sm font-semibold text-slate-800 truncate">{course.name}</h2>
            </div>
            <button
        ...
        <div className="flex-1 overflow-y-auto">
          {loading && <p className="p-4 text-xs text-slate-400">加载中...</p>}
          {notes.map((note, i) => (
            <button
              key={note.id}
        ...
        {showNewNote ? (
          <div className="max-w-2xl mx-auto px-8 py-10 animate-fade-in">
            ...
            <div className="mt-6 flex gap-3">
              <button onClick={handleSaveNew} className="px-5 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 transition-colors">保存</button>
              <button onClick={() => { setShowNewNote(false); setIsEditing(false) }} className="px-5 py-2 text-slate-500 text-sm hover:text-slate-700 transition-colors">取消</button>
            </div>
          </div>
        ) : currentNote ? (
          <div className="max-w-2xl mx-auto px-8 py-10 animate-fade-in">
                ...
                <div className="flex items-start justify-between mb-1">
                  ...
                  <div className="flex items-center gap-1 shrink-0 ml-4">
                    {currentNote.file_type && (
                      <>
                        {/* Preview button: works for all document types (PPTX/DOCX use converted PDF) */}
                        <button
                          onClick={() => {
                        ...
                        {/* Download original button for PPTX/DOCX */}
